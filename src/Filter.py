...
class WordFilter:
    def __init__(self):
        self.stopwords_ = [word.capitalize() for word in stopwords.words("german")]


    def filter_words(self, words: Dict[str, List[Slide]]) -> Dict[str, List[Slide]]:
        max_page_number = self.get_max_page_number(words)
        removed_words = {}
        for word in list(words):
            if self.should_be_removed(word) or not self.is_dense(words[word], max_page_number):
                removed_slides = words.pop(word)
                removed_words[word] = removed_slides
        self.print_removed_words(removed_words)
        return words


    def get_max_page_number(self, words_dict):
        max = 0
    ...
    def should_be_removed(self, word):
    ...
    def is_dense(self, slides, max_page_number):
        if len(slides) == 1:
            return True
        new_streak_after = 10
        max_num_streaks = 5
    ...
    def print_removed_words(self, removed_words):
        index = {}
